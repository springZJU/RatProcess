...
MATPATH{1} = 'E:\ratNeuroPixel\matData\ECoGRat1\ER320221226\ECoG1\clickTrainLongTerm_Ratio_2_v1_1\data.mat';
...
colorGrad = ["#AAAAAA", "#000000", "#0000FF", "#00FF00", "#FFA500", "#FF0000"];
...
for mIndex = 1 : length(MATPATH)
    ...
    posStr = temp(6);
    protocolStr = temp(7);
    FIGPATH = strcat(ROOTPATH, "Figure_Ratio_Awake_Aneasthetic\", protocolStr, "\", DateStr, "\");
    ...
    if doICA
        mkdir(SAVEPATH);
         ...
         ICAName = strcat(SAVEPATH, "IC_Comp_Merge",  ".mat");
...
%         Fig(dIndex, mIndex) = plotRawWave_RatECOG(chLfpMean{dIndex}, [], window, strcat(stateStr(mIndex), "_", stimStr(dIndex)));
    ...
    FigWave(mIndex) = plotRawWaveMulti_RatECOG(RegRatio, window);
    scaleAxes(FigWave(mIndex), "x", [-100, 600]);
    scaleAxes(FigWave(mIndex), "y", "on");
    %     addLegend2Fig(FigWave(mIndex), stimStr);
    orderLine(FigWave(mIndex), "LineStyle", "--", "bottom");
    AxesLegend = mSubplot(FigWave(mIndex), 6, 6, 6, 1);
    set(AxesLegend, "Visible", "off");
    legendStr = cellstr(stimStr);
    legend(AxesLegend,flip(FigWave(mIndex).Children(end).Children(1:end-1)), legendStr, "NumColumns", 1, "FontSize", 10);
%     print(FigWave(mIndex), strcat(FIGPATH,  stateStr(mIndex)), "-djpeg", "-r200");
