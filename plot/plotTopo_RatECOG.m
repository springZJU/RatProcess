function Fig = plotTopo_RatECOG(comp, topoSize, plotSize, ICs)
    narginchk(1, 4);
    ...
    topoTemp = comp.topo;
    temp = zeros(size(topoTemp, 1), numel(ICs));
    temp(:, [2:5, 7:30, 32:35]) = topoTemp;
    topo = zeros(numel(ICs), numel(ICs));
    topo([2:5, 7:30, 32:35], :) = temp;
    IC_Name = [0, 1:4, 0, 5:28, 0, 29:32, 0];
    for rIndex = 1:plotSize(1)
        ...
        for cIndex = 1:plotSize(2)
            ...
            [~, idx] = max(topo(:, ICNum));
            title(['IC ', num2str(IC_Name(ICNum)), ' | max - ', IC_Name(idx)]);
            xlim([1 topoSize(1)]);
            ylim([1 topoSize(2)]);
            xticklabels('');
            yticklabels('');
